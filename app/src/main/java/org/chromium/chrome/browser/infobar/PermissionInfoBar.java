...
public class PermissionInfoBar extends ConfirmInfoBar {
    ...
    private static PermissionInfoBar create(WindowAndroid windowAndroid, int enumeratedIconId,
        ...
        if (!isIncognito && contentSettings.length == 1) {
             if(contentSettings[0] == ContentSettingsType.CONTENT_SETTINGS_TYPE_GEOLOCATION) {
                 infoBar = new BrowserPermissionInfoBar(drawableId, iconBitmap, message, linkText,
                                buttonOk, buttonCancel, showPersistenceToggle);
            }
        }
