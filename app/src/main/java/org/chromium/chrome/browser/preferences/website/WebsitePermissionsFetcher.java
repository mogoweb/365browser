...
public class WebsitePermissionsFetcher {
    ...
    public void fetchAllPreferences() {
        ...
        // Secure Connect permission is per-origin
        queue.add(new SecureConnectInfoFetcher());
    ...
    public void fetchPreferencesForCategory(SiteSettingsCategory category) {
        ...
        if (category.showGeolocationSites()) {
            ...
            // WebDefender permission is per-origin
            queue.add(new WebDefenderInfoFetcher());
        } else if (category.showSecureConnectSites()) {
            // Secure Connect permission is per-origin
            queue.add(new SecureConnectInfoFetcher());
    ...
    private class WebRefinerInfoFetcher extends Task {
        ...
        public void run() {
            for (WebRefinerInfo info : WebsitePreferenceBridge.getWebRefinerInfo()) {
                ...
                if (origin == null) continue;
                WebsiteAddress embedder = WebsiteAddress.create(info.getEmbedder());
                findOrCreateSite(origin, embedder).setWebRefinerInfo(info);
            }
        }
    }
    ...
    private class WebDefenderInfoFetcher extends Task {
        ...
        public void run() {
            for (WebDefenderInfo info : WebsitePreferenceBridge.getWebDefenderInfo()) {
                ...
                if (origin == null) continue;
                WebsiteAddress embedder = WebsiteAddress.create(info.getEmbedder());
                findOrCreateSite(origin, embedder).setWebDefenderInfo(info);
            }
        }
    }

    private class SecureConnectInfoFetcher extends Task {
        @Override
        public void run() {
            for (SecureConnectInfo info : WebsitePreferenceBridge.getSecureConnectInfo()) {
                if (info.getOrigin() == null) continue;
                if (info.getOrigin().equals("*")) continue; //Default setting, so ignore
                WebsiteAddress origin = WebsiteAddress.create(info.getOrigin());
                if (origin == null) continue;
                WebsiteAddress embedder = WebsiteAddress.create(info.getEmbedder());
                findOrCreateSite(origin, embedder).setSecureConnectInfo(info);
            }
        }
    }
}
